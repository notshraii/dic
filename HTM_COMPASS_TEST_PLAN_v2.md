...
- **DICOM Routing Rules**  
  Verification of C-STORE routing logic based on AE Title, DICOM Tags (Body Part, Modality, Referring Physician), and Time Schedules. Automated tests validate that Compass accepts connections from different Calling AE Titles (CT_SCANNER_1, MR_SCANNER_A, CR_ROOM_1, US_PORTABLE, ULTRA_MCR_FORUM) and routes studies according to configured rules.

- **Tag Morphing**  
  Validation of "Composer" and "Filter" actions to ensure DICOM headers are modified correctly before reaching destinations (e.g., Patient ID normalization). De-identification, and re-identification of tags. Automated tests verify transformations such as:
  - OPV modality with GPA/SFA series description setting Visual Fields study description
  - OPT modality setting Optical Coherence Tomography study description  
  - OT modality setting IOL Master study description
  - DOC modality setting OCT and VF Combined Report description

- **Validation of file pickup consistency and stability times.**

- **Performance**
  - Load stability testing at 3x peak load (configurable via LOAD_MULTIPLIER)
  - Throughput testing at 150% and 200% of peak images per second
  - Concurrent scanner simulation using multi-threaded transmission
  - P95 latency validation against configurable thresholds (default: 2000ms for stability, 1500ms for throughput)
  - Error rate validation against configurable threshold (default: 2%)

- **Data Validation**
  - Blank StudyDate/Time handling - verify Compass populates acquisition date/time
  - Accession number generation via IIMS web service when blank
  - Accession number preservation when provided by device
  - Patient demographics edge cases (blank names, special characters, Unicode)
  - Missing required DICOM tag handling (Modality)

- **Failure Mode Testing**
  - Delay tolerance - 2-minute pauses between file sends
  - Duplicate study handling - same StudyInstanceUID sent multiple times
  - Modified duplicate handling - resend with different patient data
  - Variable network delays (5-60 second random intervals)
