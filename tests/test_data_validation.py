...
def test_pass_device_accession_number(
    ...
    """
    COMPASS_DataValidation_AccessionNumberGenerated
    
    Send study with blank accession number (blanked automatically by the
    sender). Verify IIMS generates and populates an accession number.
    
    Expected Result: IIMS web service generates an accession number
    
    Test Steps:
    1. Send study (AccessionNumber blanked by sender)
    2. C-FIND verification: confirm AccessionNumber was populated
    """
    ds = load_dataset(single_dicom_file)
    ...
    print(f"\n{'='*70}")
    print(f"ACCESSION NUMBER GENERATION TEST")
    print(f"{'='*70}")
    print(f"  StudyInstanceUID: {ds.StudyInstanceUID}")
    print(f"  AccessionNumber: '' (blanked by sender, expecting IIMS generation)")
    ...
    with manual_verification_required("AccessionNumber generation -- verified manually on server"):
        assert acc and acc.strip(), (
            f"AccessionNumber was not populated by IIMS for study {ds.StudyInstanceUID}. "
            f"Sent with blank AccessionNumber, expected IIMS to generate one. "
            f"Strategy used: '{strategy}'. "
            f"C-FIND response keys: {list(study.keys())}."
        )
    print(f"  [OK] AccessionNumber generated by IIMS: {acc}")
